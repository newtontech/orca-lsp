# Agent Verification Loop for ORCA LSP

## Overview

This document describes the recommended verification loop for coding agents
(Claude Code, OpenCode, Codex) working with ORCA input files using the
ORCA LSP server.
...
The ORCA LSP provides custom commands for agent consumption:

- `ORCA.diagnosticsSnapshot` — Returns JSON snapshot of all diagnostics.
...
```python
config = TestRunnerConfig(executable="ORCA", enabled=True)
