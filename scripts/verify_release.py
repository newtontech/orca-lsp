#!/usr/bin/env python3
"""Verify that all ORCA-LSP release metadata agrees with the pushed tag."""
...
def _versions() -> dict[str, str]:
    ...
    fixture_capabilities = json.loads(
        (ROOT / "tests/fixtures/capabilities/lsp_capabilities.json").read_text(encoding="utf-8")
    )
    init_text = (ROOT / "src/orca_lsp/__init__.py").read_text(encoding="utf-8")
    ...
    return {
        ...
        "orca_lsp fallback": init_match.group(1),
        "lsp-capabilities.json": capabilities["version"],
        "capabilities fixture": fixture_capabilities["version"],
    }
...
def main() -> int:
    ...
    parser.add_argument("--tag", required=True, help="Release tag, for example v0.5.6")
