#!/usr/bin/env python3
"""Install an ORCA-LSP wheel in isolation and smoke its public CLIs."""
...
def smoke(wheel: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orca-lsp-wheel-") as temporary:
        ...
        server = bindir / f"orca-lsp{executable_suffix}"
        tool = bindir / f"orca-lsp-tool{executable_suffix}"
        ...
        installed_version = _run(
            [
                str(python),
                "-c",
                "from importlib.metadata import version; print(version('orca-lsp'))",
            ],
            cwd=isolated,
        ).stdout.strip()
        ...
        valid = ROOT / "tests/fixtures/valid/water_optimization.inp"
        ...
        invalid = ROOT / "tests/fixtures/invalid/missing_method.inp"
        ...
        log = ROOT / "tests/fixtures/log/scf_convergence_failure.log"
        log_payload = _json(
            _run(
                [str(tool), "parse-log", str(log), "--fail-on-blocking"],
                cwd=isolated,
                expected_returncodes=(1,),
            )
        )
        if log_payload.get("operation") != "parse-log" or not log_payload.get("diagnostics"):
