"""Release contract for the ORCA-LSP 0.5.6 publication path."""
...
def test_release_version_is_consistent_across_all_manifests() -> None:
    ...
    init_text = (REPO_ROOT / "src/orca_lsp/__init__.py").read_text(encoding="utf-8")
    ...
    fixture_capabilities = json.loads(
        (REPO_ROOT / "tests/fixtures/capabilities/lsp_capabilities.json").read_text(
            encoding="utf-8"
        )
    )
